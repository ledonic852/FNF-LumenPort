--[[Recreation by Gostar64 and modified by Ledonic
	Works on Psych Engine 0.7.3 and 1.0.x [And maybe some older versions]
	Don't remove this credit thing or I'll be sad Ex: [:(]

	Also this was based off of Super_Hugo's Philly Glow Recreation,
	before I (Ledonic) modified the code to be more accurate to Psych Engine.
]]

-- This is for more advanced usage of the event in other songs and/or stages.
local extraData = {
...
function onCreate()
	--[[
		Example code (copy, paste and change whatever you need to):
...
-- Sets up the sprites for the 'Dadbattle Spotlight' event.
function onCreatePost()
	...
	smoke1OffsetY = getRandomFloat(-20, 20) + extraData.smokeOffset.y
	smoke1Scale = getRandomFloat(1.1, 1.22)
	smoke1Velocity = getRandomFloat(15, 22)
	makeLuaSprite('smoke1', 'stage/smoke', -1350 + smoke1OffsetX, 660 + smoke1OffsetY)
	...
	smoke2OffsetY = getRandomFloat(-20, 20) + extraData.smokeOffset.y
	smoke2Scale = getRandomFloat(1.1, 1.22)
	smoke2Velocity = getRandomFloat(-22, -15)
	makeLuaSprite('smoke2', 'stage/smoke', 1750 - smoke2OffsetX, 660 + smoke2OffsetY)
